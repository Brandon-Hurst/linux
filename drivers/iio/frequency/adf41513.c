...
enum {
	...
	ADF41513_FREQ_REFIN,
	ADF41513_FREQ_PFD,
	ADF41513_LOCK_DETECT_COUNT,
	ADF41513_LOCK_DETECT_COUNT_AVAILABLE,
};
...
static u64 adf41513_pll_get_rate(struct adf41513_state *st)
{
	...
	st->settings.int_value = FIELD_GET(ADF41513_REG0_INT_MSK,
					   st->regs_hw[ADF41513_REG0]);
	st->settings.frac1 = FIELD_GET(ADF41513_REG1_FRAC1_MSK,
				       st->regs_hw[ADF41513_REG1]);
	st->settings.frac2 = FIELD_GET(ADF41513_REG3_FRAC2_MSK,
				       st->regs_hw[ADF41513_REG3]);
	st->settings.mod2 = FIELD_GET(ADF41513_REG4_MOD2_MSK,
				      st->regs_hw[ADF41513_REG4]);
	st->settings.r_counter = FIELD_GET(ADF41513_REG5_R_CNT_MSK,
					   st->regs_hw[ADF41513_REG5]);
	st->settings.ref_doubler = FIELD_GET(ADF41513_REG5_REF_DOUBLER_MSK,
					     st->regs_hw[ADF41513_REG5]);
	st->settings.ref_div2 = FIELD_GET(ADF41513_REG5_RDIV2_MSK,
					  st->regs_hw[ADF41513_REG5]);
	st->settings.prescaler = FIELD_GET(ADF41513_REG5_PRESCALER_MSK,
					   st->regs_hw[ADF41513_REG5]);
	...
	if (FIELD_GET(ADF41513_REG6_INT_MODE_MSK, st->regs_hw[ADF41513_REG6])) {
		...
		if (FIELD_GET(ADF41513_REG0_VAR_MOD_MSK, st->regs_hw[ADF41513_REG0])) {
			...
			if (!FIELD_GET(ADF41513_REG5_LSB_P1_MSK, st->regs_hw[ADF41513_REG5]))
				st->settings.actual_frequency_uhz +=
					div64_u64(st->settings.pfd_frequency_uhz,
				       ...
				       u64 fpfd_limit_uhz)
{
	...
	do {
		...
		if (result->ref_div2)
			result->pfd_frequency_uhz >>= 1;
		result->pfd_frequency_uhz = div_u64(result->pfd_frequency_uhz, result->r_counter);
...
{
	u64 freq_error_uhz;
	u64 resolution_uhz = div64_u64(result->pfd_frequency_uhz, ADF41513_FIXED_MODULUS);
	...
	freq_error_uhz -= mul_u64_u64_div_u64((u64)frac1, result->pfd_frequency_uhz,
					      ADF41513_FIXED_MODULUS);
	...
	result->actual_frequency_uhz += mul_u64_u64_div_u64((u64)frac1,
							    result->pfd_frequency_uhz,
							    ADF41513_FIXED_MODULUS);
				      ...
				      struct adf41513_pll_settings *result)
{
	...
	mod2 = DIV64_U64_ROUND_CLOSEST(result->pfd_frequency_uhz * 2,
				       st->data.freq_resolution_uhz * ADF41513_FIXED_MODULUS);
	mod2 = min(mod2, ADF41513_MAX_MOD2);
	...
	freq_error_uhz -= mul_u64_u64_div_u64((u64)frac1, result->pfd_frequency_uhz,
					      ADF41513_FIXED_MODULUS);
	frac2 = mul_u64_u64_div_u64(freq_error_uhz, ADF41513_FIXED_MODULUS * mod2,
				    result->pfd_frequency_uhz);
...
static void adf41513_set_bleed_val(struct adf41513_state *st)
{
	...
	bleed_value = mul_u64_u64_div_u64(st->settings.pfd_frequency_uhz,
					  (u64)bleed_value,
					  16 * 100ULL * HZ_PER_MHZ * MICROHZ_PER_HZ);

	st->regs[ADF41513_REG6] &= ~ADF41513_REG6_BLEED_CURRENT_MSK;
	st->regs[ADF41513_REG6] |= FIELD_PREP(ADF41513_REG6_BLEED_CURRENT_MSK, bleed_value);
}
...
static int adf41513_set_frequency(struct adf41513_state *st, u64 freq_uhz, u16 sync_mask)
{
	...
	/* int */
	st->regs[ADF41513_REG0] = FIELD_PREP(ADF41513_REG0_INT_MSK, st->settings.int_value);
	if (st->settings.mode == ADF41513_MODE_VARIABLE_MODULUS)
		st->regs[ADF41513_REG0] |= ADF41513_REG0_VAR_MOD_MSK;
	/* frac1 */
	st->regs[ADF41513_REG1] = FIELD_PREP(ADF41513_REG1_FRAC1_MSK, st->settings.frac1);
	if (st->settings.mode != ADF41513_MODE_INTEGER_N)
		st->regs[ADF41513_REG1] |= ADF41513_REG1_DITHER2_MSK;

	/* frac2 */
	st->regs[ADF41513_REG3] = FIELD_PREP(ADF41513_REG3_FRAC2_MSK, st->settings.frac2);
	/* mod2 */
	st->regs[ADF41513_REG4] &= ADF41513_REG4_MOD2_MSK;
	st->regs[ADF41513_REG4] |= FIELD_PREP(ADF41513_REG4_MOD2_MSK, st->settings.mod2);
				 ...
				 char *buf)
{
	...
	case ADF41513_FREQ_RESOLUTION:
		...
		break;
	case ADF41513_FREQ_PFD:
		adf41513_pll_get_rate(st);
		freq_uhz = st->settings.pfd_frequency_uhz;
		break;
	default:
		return -EINVAL;
	}
			     ...
			     char *buf)
{
	...
	case ADF41513_POWER_DOWN:
		...
		break;
	case ADF41513_LOCK_DETECT_COUNT:
		val = st->data.lock_detect_count;
		break;
	case ADF41513_LOCK_DETECT_COUNT_AVAILABLE:
		return sysfs_emit(buf, "2 4 8 16 32 64 128 256 512 1024 2048 4096 8192\n");
	default:
		return -EINVAL;
	}
				  ...
				  const char *buf, size_t len)
{
	...
	case ADF41513_FREQ:
		...
		break;
	case ADF41513_FREQ_RESOLUTION:
		if (freq_uhz == 0 || freq_uhz > ADF41513_MAX_PFD_FREQ_INT_N_UHZ)
			ret = -EINVAL;
		else
			      ...
			      const char *buf, size_t len)
{
	...
	case ADF41513_POWER_DOWN:
		...
		break;
	case ADF41513_LOCK_DETECT_COUNT:
		if (readin < ADF41513_LD_COUNT_FAST_MIN || readin > ADF41513_LD_COUNT_MAX ||
		    !is_power_of_2(readin)) {
			ret = -EINVAL;
			break;
		}

		if (readin >= ADF41513_LD_COUNT_FAST_LIMIT) {
			tmp = ilog2(readin) - const_ilog2(ADF41513_LD_COUNT_MIN);
			st->regs[ADF41513_REG7] &= ~ADF41513_REG7_LD_CLK_SEL_MSK;
		} else {
			tmp = ilog2(readin) - const_ilog2(ADF41513_LD_COUNT_FAST_MIN);
			st->regs[ADF41513_REG7] |= ADF41513_REG7_LD_CLK_SEL_MSK;
		}

		st->regs[ADF41513_REG7] &= ~ADF41513_REG7_LD_COUNT_MSK;
		st->regs[ADF41513_REG7] |= FIELD_PREP(ADF41513_REG7_LD_COUNT_MSK, tmp);
		ret = adf41513_sync_config(st, ADF41513_SYNC_DIFF);
		if (!ret)
			st->data.lock_detect_count = readin;
		break;
	default:
		ret = -EINVAL;
		break;
...
static const struct iio_chan_spec_ext_info adf41513_ext_info[] = {
	...
	_ADF41513_EXT_UHZ_INFO("pfd_frequency", ADF41513_FREQ_PFD),
	_ADF41513_EXT_INFO("refin_frequency", ADF41513_FREQ_REFIN),
	_ADF41513_EXT_INFO("powerdown", ADF41513_POWER_DOWN),
	_ADF41513_EXT_INFO("ld_count", ADF41513_LOCK_DETECT_COUNT),
	_ADF41513_EXT_INFO("ld_count_available", ADF41513_LOCK_DETECT_COUNT_AVAILABLE),
	{ },
};
			     ...
			     int *val, int *val2, long info)
{
	...
	case IIO_CHAN_INFO_PHASE:
		...
		phase_udeg = (360ULL * MICRO * phase_val) >> 12;
		*val = phase_udeg / MICRO;
		*val2 = phase_udeg % MICRO;
			      ...
			      int val, int val2, long info)
{
	...
	case IIO_CHAN_INFO_PHASE:
		...
		if (val < 0)
			val += 360;
		phase_udeg = val * MICRO + val2;
		phase_val = div_u64(phase_udeg << 12, (360UL * MICRO));
...
static int adf41513_setup(struct adf41513_state *st)
{
	...
	st->regs[ADF41513_REG5] = FIELD_PREP(ADF41513_REG5_DLD_MODES_MSK, ADF41513_DLD_DIG_LD);

	/* configure charge pump current settings */
	cp_index = DIV_ROUND_CLOSEST(st->data.charge_pump_voltage_mv, ADF41513_MIN_CP_VOLTAGE_mV);
	st->regs[ADF41513_REG5] |= FIELD_PREP(ADF41513_REG5_CP_CURRENT_MSK, cp_index - 1);
	...
	/* muxout */
	st->regs[ADF41513_REG12] = FIELD_PREP(ADF41513_REG12_MUXOUT_MSK, st->data.muxout_select);
